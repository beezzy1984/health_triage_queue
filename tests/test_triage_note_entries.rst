...
Get Triage Model::
    ...
    >>> Note = Model.get('gnuhealth.triage.note')
...
Create Note::
    ...
    >>> note.triage_entry = triage

    >>> note.note = ' '.join(['Som random texts', str(datetime.now())])
...
Test Scenario::
    ...
    >>> triage.note_entries == None and len(triage.note_entries) > 1
    False
